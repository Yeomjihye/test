...
window.addEventListener('load', () => {
	...
	inputEL.addEventListener('keydown', function (evt) {
	    if(evt.key === 'Enter') {
	    	evt.preventDefault(); // 서버 못 가게 막는것
...
function searchList() {
	...
	if(! f.kwd.value.trim()) { // 입력된 내용이 없으면
		return;
	}
	...
	let requestParams = new URLSearchParams(formData).toString();
		// formData 만 넣을 수 있고 f는 넣을 수 없다	
	// get 방식의 &,% 는 안 넘어가기 때문에 인코딩해야된다
	// 인코딩 안하기 위해서 URLSearchParams 이걸 쓰면 된다
